...
- `Account::PasswordHash.authenticate_by` and `#recover!`/`PasswordHash#generate` are constant-time with respect to account/password state: an account that doesn't exist, an account that exists but has no password, and a blank password input all cost the same as a real failed-password attempt, so none of them can be distinguished from each other by response time.
