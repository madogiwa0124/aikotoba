...
- Add opt-in `request_back_after_sign_in` scope config: when `true`, a successful sign in (password or [MagicLinkAuthenticatable](README.md#experimental-magiclinkauthenticatable)) redirects back to whatever page the account was trying to reach before being sent to sign in, instead of always going to `after_sign_in_path`. No host app changes are needed — Aikotoba captures the destination from the `Referer` header on the `GET /sign_in` request that the host app's own `authenticate_account!`-style redirect produces. Only a same-host path is ever trusted and stored, and it's consumed at most once, so a spoofed or stale `Referer` can't be replayed or turned into an open redirect. Defaults to `false`. See the README's [Authenticatable](README.md#authenticatable) section.
