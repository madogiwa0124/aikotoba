...
module Aikotoba
  class Account::MagicLinkToken < ApplicationRecord
    ...
    after_initialize do |record|
      token = Account::Token.new(expiry: Aikotoba.magic_link_token_expiry)
      record.token ||= token.value
      record.expired_at ||= token.expired_at
    end

    def notify
      AccountMailer.with(account: account).magic_link.deliver_now
    end
  end
end
