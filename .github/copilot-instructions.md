...
- Storage + matching: [app/models/aikotoba/account/password.rb](../app/models/aikotoba/account/password.rb) — `Aikotoba::Account::Password` (table `aikotoba_account_passwords`) owns the password digest and `#match?`. `Account` itself has no password-shaped methods at all, only a `has_one :password` association (`account.password.value`/`.digest`). `Registrable#build_by` is the one place that still knows the `:password` key exists, translating it into `build_password(value:)` so the flat `account[password]` form/params contract stays unchanged.
